...
def get_named_entities(text):
    ...
    nlp_text = nlp(text)
    return [str(ent).lower() for ent in nlp_text.ents]
...
for article in news:
    named_ent = get_named_entities(utils.to_unicode(article['text']))
    named_ent = ' '.join(named_ent).split()
    tokens = utils.to_unicode(article['new_text']).split()
    sentences.append(TaggedDocument(tokens, named_ent))
...
embedding_size = 200
text_model = None
filename = 'similarity_doc2vec_ner_data.d2v'
